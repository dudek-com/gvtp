...
spp_native <- spp %>% 
  ...
  mutate(
    native_ratio = NATIVE / (NATIVE + NATURALIZED)) %>% 
  arrange(desc(native_ratio))
View(spp_native)
...
plot_native %>%
  select(plot, gvtp, gvtp_count, native_ratio) %>% 
  arrange(gvtp, native_ratio) %>% 
  View()
  
# d_avg <- plot_native %>% gro
  
ggplot(plot_native, aes(x=native_ratio, fill=gvtp, color=gvtp)) +
  geom_density(alpha=0.2) +
  geom_histogram(
    aes(y=..density..), position="identity", alpha=0.5) 
  # geom_vline(
